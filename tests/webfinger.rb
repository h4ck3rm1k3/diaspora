...
class Webfinger
  ...
  def initialize(account)
    ...
    @ssl = true
   print ("event=webfinger status=initialized target=#{account}\n")
  end
...
#     end
      ...
      if fingered_person
        print ("event=webfinger status=success route=remote target=#{@account}\n")
        fingered_person
      else
        print ("event=webfinger status=failure route=remote target=#{@account}\n")
        raise WebfingerFailedError.new(@account)
      end
#  rescue Exception => e
#     print ("event=receive status=abort recipient=#{@account} reason='#{e.message}'\n")
#     nil
    end
  end
  ...
  def get_xrd
    begin
      print "going to get " + xrd_url + "\n"
      http = Faraday.get xrd_url
      print "got body " + http.body
  ...
  def get_webfinger_profile(profile_url)
    begin
      print "get profile url" + profile_url + "\n"
      http = Faraday.get(profile_url)
      print "got profile url" + http.body + "\n"
  ...
  def get_hcard(webfinger_profile)
    unless webfinger_profile.strip == ""
      ...
      begin
        print "get hcard url" + hcard_url + "\n"
        hcard = Faraday.get(hcard_url)
        
        print "got hcard url" + hcard.body + "\n"

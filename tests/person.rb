...
#  validates :profile, :presence => true
  ...
  def serialized_public_key(x)
    @serialized_public_key=x
  end
  def serialized_public_key=(x)
    @serialized_public_key=x
  end
...
#  scope :all_from_aspects, lambda { |aspect_ids, user|    joins(:contacts => :aspect_memberships).         where(:contacts => {:user_id => user.id},               :aspect_memberships => {:aspect_id => aspect_ids}).         select("DISTINCT people.*")  }
...
#    self.where(:diaspora_handle => identifier).first
  ...
  def self.create_from_webfinger(profile, hcard)
    ...
    print("event=webfinger_marshal valid=#{new_person.valid?} target=#{new_person.diaspora_handle}\n")
